...
for insect_simulation in insects_simulations:
    if insect_simulation == 'musca-simulations': #this one can be commented out. it's currently the test folder but if commented out, all folders will be parsed. 
        ...
        for case_study in case_studies:
            ...
            # exit()
            # if case_study != 'diptera-simulation': 
            #     continue
...
#                 # exit()
#                 for cfd_run in cfd_runs:
#                     post_processing_coefficients = os.path.join('/Users/nico/Documents/GitHub/QSM-insects/post-processing/diptera/diptera-simulation/', cfd_run)
#                     cfd_run_path = os.path.join(simulations_folder_path, insect_simulation, case_study, cfd_run)
#                     # print(cfd_run_path)
#                     # exit()
#                     if os.path.isdir(cfd_run_path):
#                         folder_name = f'post-processing/{insect_simulation}/{case_study}/{cfd_run}/{rightnow}'
#                         post_processing_coefficients = f'post-processing/{insect_simulation}/diptera-simulation/{cfd_run}/'
#                         try:
...
#                             post_processing_coefficients = f'post-processing/{insect_simulation}/diptera-simulation/{cfd_run}/{latest_run}'
#                             make_dir(folder_name)
#                             x0_forces = readArrayfromCSV(post_processing_coefficients+'/force_coefficients.csv')[1].flatten()[:-1]
#                             x0_moments = readArrayfromCSV(post_processing_coefficients+'/moment_coefficients.csv')[1].flatten()[:-1]
#                             x0_power = readArrayfromCSV(post_processing_coefficients+'/power_coefficients.csv')[1].flatten()[:-1]
