...
def make_dir(dir_name, index=0):
    split = dir_name.split('/')  #the function counts the # of items seperated by a '/' and stores them in a list 'split' 
    if len(split) >= index: #if len(split)  is greater than index, it means there is more items to process than have been so far
        # print(len(split))
        # print(index)
        # print(split)
        new = '/'.join(split[:-1]) #a directory called 'new' is then created and all names till index+1 are stuck together
