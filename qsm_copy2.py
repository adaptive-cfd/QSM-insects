...
if 'from_file' in wingShape:
    wingShape_file = os.path.join('phi120.00_phim20.00_dTau0.05', wingShape.replace('from_file::', ''))
...
if 'from_file' in kinematics_file:
    kinematics_file = os.path.join('phi120.00_phim20.00_dTau0.05', kinematics_file.replace('from_file::', ''))
...
def generateSequence():
    for timeStep in range(nt):
        global strokePointsSequence, planar_rots_wing_w
...
# #variables that require elements from kinematics() are computed here. 
# for i in range(nt):
#     M_CFD_w[i, :] =  np.matmul(wingRotationMatrix_sequence[i],np.matmul(strokeRotationMatrix_sequence[i], np.matmul(bodyRotationMatrix_sequence[i], M_CFD[i])))
############################################################################################################################################################################################
##% dynamics
...
def cost(x, numerical=False, nb=1000, show_plots=False):
    ...
    if numerical:
        ...
        for i in range(y_space.shape[0]):
            ...
            # blade_us_wing_w = np.cross(_rots_wing_w, y_blade_w)
            ...
            # Fam_magnitude += Cam1*phis_dt_dt_sequence*r*c[i]**2*dr*np.cos(alphas) + Cam2*phis_dt_dt_sequence*r*c[i]**2*dr*np.sin(alphas) #following van Veen et al. 2022 #with Cam1 = CFam,x and Cam2 = CFam,z
