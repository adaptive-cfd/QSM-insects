...
def generateSequence():
    for timeStep in range(nt):
        ...
        # print(np.matmul(wingRotationMatrixTrans, np.array([[0], [0], [1]]))) 
        # exit()
...
def cost(x, numerical=False, nb=1000, show_plots=False):
    ...
    Cl, Cd, Crot, Cam1, Cam2 = getAerodynamicCoefficients(x, np.array(AoA))
    ...
    if numerical:
        ...
        for i in range(y_space.shape[0]):
            ...
            # blade_us_wing_g = np.cross(_rots_wing_g, y_blade_g)
            ...
            # for j in range(nt):
                ...
                # blade_acc_wing_w[j, :] = (blade_us_wing_w[(j+1)%nt] - blade_us_wing_w[(j-1)%nt])/2*delta_t    
    ...
    if show_plots:
        ...
        gCl, gCd, gCrot, gCam1, gCam2 = getAerodynamicCoefficients(x, graphAoA)
        fig, ax = plt.subplots()
