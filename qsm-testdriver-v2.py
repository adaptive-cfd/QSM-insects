...
Q.append_KinematicsShapeForces_fromCFDrun( run, T_start=1.0, T_end=2.0, dt=QSM.dt, wing='right', 
                                          verbose=True)#, wingShapeFile=wingShapeFile)

Q.fit_to_CFD(optimize=True, N_trials=5)
...
_, alpha, phi, theta = insect_tools.bumblebee_kinematics_model(PHI=140, phi_m=0.0, alpha_down=45.0, 
                                                               alpha_up=-45.0, time=t)
...
plt.figure()
plt.plot( QSM.timeline    , QSM.F_QSM_g[:,2], label='QSMv1 vertical force, reference configuration')
plt.plot( QSM_new.timeline, QSM_new.F_QSM_g[:,2], label='QSMv1 vertical force, modified kinematics (prediction)')
plt.plot( Q.timeline      , Q.F_QSM_g[:,2], '--', label='QSMv2 vertical force, reference configuration')
plt.plot( Q2.timeline     , Q2.F_QSM_g[:,2], '--', label='QSMv2 vertical force, modified kinematics (prediction)')
plt.legend()
