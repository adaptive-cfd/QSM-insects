...
now = datetime.now()
today_filename =now.strftime(" %d-%m-%Y %I:%M:%S")+".png"
...
for timeStep in range(nt):
    ...
    # acc_wing_w[timeStep, :] = (us_wing_w[(timeStep+1)%nt] - us_wing_w[timeStep-1])/(2*delta_t) #acc_wing_w_2
...
# plt.legend()
# plt.savefig('debug_images/cfd moments_w insect_tools vs own'+today_filename, dpi=300)
# plt.show()
# exit()
...
def cost(x, nb=1000, show_plots=False):
    ...
    if show_plots:
        ...
        plt.legend()
        # plt.savefig('debug_images/forces; no Fwe; acc_wing_w_2; '+cfd_run+today_filename, dpi=300)
        plt.show()
